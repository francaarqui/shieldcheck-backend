...
/* ==================== GENERIC SECURITY ITEM CHECK ==================== */

app.get('/api/check-item', optionalAuthenticateToken, async (req, res) => {
    const { value, type } = req.query; // type: 'pix', 'phone', 'email'
    if (!value) return res.status(400).json({ error: 'Valor obrigatório.' });

    console.log(`🔍 Verificando item de segurança: [${type}] ${value}`);

    try {
        // 1. Verificar em scam_patterns (Crowdsourcing)
        const { data: pattern } = await supabase
            .from('scam_patterns')
            .select('report_count, pattern_type')
            .eq('pattern_text', value.trim())
            .maybeSingle();

        let crowdResult = {
            isReported: !!pattern,
            reportCount: pattern ? pattern.report_count : 0
        };

        // 2. Usar IA para análise heurística baseada no tipo
        const prompt = `
        Aja como um analista de segurança digital. Analise o seguinte item do tipo "${type}": "${value}".
        Forneça um score de risco de 0 a 100 e uma recomendação.
        Se for PIX, considere se a chave parece gerada aleatoriamente para golpes ou se é um CPF/E-mail comum.
        Se for Telefone, considere se o formato é válido para o Brasil.
        Mencione o fato de já ter sido reportado ${crowdResult.reportCount} vezes por outros usuários.
        ...
        // Mesclar resultados
        const finalScore = Math.min(100, aiResult.score + (crowdResult.reportCount * 20));

        res.json({
            ...aiResult,
            score: finalScore,
            reportedTimes: crowdResult.reportCount,
            status: finalScore > 60 ? 'ALTO RISCO' : finalScore > 30 ? 'SUSPEITO' : 'BAIXO RISCO'
        });
...
app.get('/api/expand-url', optionalAuthenticateToken, async (req, res) => {
    ...
    try {
        ...
        // Analisar o destino final
        const analysis = await analyzeStore(finalUrl);
        ...
        res.status(500).json({ error: 'Não foi possível expandir a URL.' });
    }
});
...
// ============================================================
// B2B LEADS - Formulário de contato empresarial
// ============================================================
app.post('/api/leads/b2b', async (req, res) => {
    ...
    try {
        ...
        if (error) {
            // Se a tabela não existe, registramos o lead no console mas retornamos sucesso
            console.log('📋 Novo lead B2B (tabela leads não encontrada, verifique o Supabase):', { name, company, email, business_type });
            console.error('Supabase leads error:', error.message);
        } else {
            console.log(`📋 Novo lead B2B: ${name} - ${company} (${email})`);
        }
